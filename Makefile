...
run: env-check ## Run the agent locally
	@echo "🤖 Starting financial-coach-agent..."
	@uv run python -m financial_coach_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting financial-coach-agent in development mode..."
	@uv run watchfiles "python -m financial_coach_agent" financial_coach_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t financial_coach_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t financial_coach_agent:latest .
...
docker-stop: ## Stop Docker container
	...
	@docker stop financial_coach_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm financial_coach_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f financial_coach_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it financial_coach_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi financial_coach_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag financial_coach_agent:latest para5/financial_coach_agent:latest
	@docker push para5/financial_coach_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep financial_coach_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep financial_coach_agent || echo "No images found"
	@echo "\nAgent Health:"
